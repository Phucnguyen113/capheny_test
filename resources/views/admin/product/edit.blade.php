...
<div class="main-card mb-3 card">
    <div class="card-body"><h5 class="card-title" style="font-size:36px;text-align:center">Chỉnh sửa sản phẩm</h5>
        <form class="" enctype="multipart/form-data" onsubmit="return edit_product()">
            @csrf
            <!-- name product -->
            <div class="form-row">
                <div class="col-md-4">
                    <div class="position-relative form-group">
                        ...
                        <input name="product_name" value="{{$product->product_name}}" onkeyup="convert_vn_to_en()" id="product_name" placeholder="Tên sản phẩm" type="text" class="form-control">
                ...
                <div class="col-md-4">
                    <div class="position-relative form-group">
                        ...
                        <input name="product_slug" value="{{$product->product_slug}}" id="product_slug" placeholder="Đường dẫn sản phẩm" type="text" class="form-control">
                ...
                <div class="col-md-4">
                    <div class="position-relative form-group">
                        ...
                        <script>
                            ...
                            // get list category selected
                            var category_detail=[];
                            $.ajax({
                                type: "post",
                                url: "{{url('product/get_category')}}/{{$product->product_id}}",
                                data: {_token:$('input[name="_token"]').val()},
                                async:false,
                                dataType: "json",
                                success: function (response) {
                                    category_detail=response
                                }
                            });
            ...
            <div class="form-row">
                <div class="col-md-3">
                    <div class="position-relative form-group">
                        <label for="discount_type_old" class="">Giảm giá hiện tại</label>
                        <select name="discount_type_old" id="discount_type_old" class="form-control" disabled>
                            <option @if($product->discount_type==0) selected @endif value="0">Không giảm giá</option>
                            <option @if($product->discount_type==1) selected @endif value="1">Giảm tiền cố định</option>
                            <option @if($product->discount_type==2) selected @endif value="2">Giảm theo % giá bán</option>
                        </select>
                        <div id="discount_type_error_old" class="p_error"></div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="position-relative form-group">
                        <label for="discount_amount_old" class="">Số tiền giảm</label>
                        <input name="discount_amount_old" value="{{$product->discount_amount}}" disabled id="discount_amount_old" type="text" class="form-control">
                        <div id="discount_amount_error_old" class="p_error"></div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="position-relative form-group">
                        <label for="discount_from_date_old" class="">Ngày bắt đầu giảm</label>
                        <input name="discount_from_date_old" disabled value="{{$product->discount_from_date}}" id="discount_from_date_old" placeholder="Ngày sửa"  readonly='true' type="text" class="form-control">
                        <div id="picker_time_discount_from_date_old"   style="display:flex;justify-content:center;position:absolute;z-index:99"></div>
                        <div id="discount_from_date_error_old" class="p_error" style="color:red"></div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="position-relative form-group">
                        <label for="discount_end_date_old" class="">Ngày kết thúc giảm</label>
                        <input name="discount_end_date_old" disabled value="{{$product->discount_end_date}}" id="discount_end_date_old" placeholder="Ngày sửa" readonly='true'  type="text" class="form-control">
                        <div id="picker_time_discount_end_date_old" style="display:flex;justify-content:center;position:absolute;z-index:99"></div>
                        <div id="discount_end_date_error_old"  style="color:red"></div>
                    </div>
                </div>
            </div>
            <!-- discount -->
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-3">
                                <div class="position-relative form-group">
                                    ...
                                    <input name="discount_amount" disabled id="discount_amount" type="text" class="form-control">
            ...
            <div class="form-row" style="margin-top:20px">
                <div class="col-md-3">
                    ...
                    <input type="text" style="text-align:right" value="{{$product->product_price}}" name="product_price" id="product_price" class="form-control">
                ...
                <!-- <div class="col-md-3">
                    ...
                    <select class="select-2 js-example-basic-multiple form-control" id="color" style="width:100%" multiple name="color[]">
                        @foreach($color as $key =>$value)
                            @php 
                                $symple='';
                                foreach($product->color as $colors =>$color){
                            ...
                            <option  {{$symple}} value="{{$value->color_id}}" data-color="{{$value->color}}"></option>
                ...
                <!-- <div class="col-md-3">
                    ...
                    <select class="select-2-size js-example-basic-multiple form-control" id="size" style="width:100%" multiple name="size[]">
                        @foreach($size as $key =>$value)
                            @php 
                                $symple='';
                                foreach($product->size as $sizes =>$size){
                                    if($value->size_id== $size->size_id){
                                        $symple='selected';
                                        break;
                                    } 
                                }
                            @endphp
                                <option {{$symple}} value="{{$value->size_id}}" >{{$value->size}}</option>
                    ...
                    <div class="position-relative form-group">    
                        <div class="form-check">
                            <label class="form-check-label">
                                <input type="checkbox" @if($product->active==1) checked @endif class="form-check-input" name="active" id="active" value="1" >
                                Kích hoạt
                            </label>
                        </div>
                    </div>
            ...
            <button class="mt-2 btn btn-primary">Sửa</button>
        </form>
    </div>
</div>
<!-- edit product script -->
<script>
    function edit_product(){
        ...
        $.ajax({
            type: "post",
            url: "{{url('admin/product')}}/{{$product->product_id}}",
            data: formdata,
            contentType: false,
            processData: false,
            dataType: "json",
